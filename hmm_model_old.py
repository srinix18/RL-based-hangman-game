    ...
    def predict_letter_probabilities(self, masked_word, guessed_letters):
        ...
        for word in candidates:
            for letter in set(word):  # Count each letter once per word
                # Only count valid uppercase letters that haven't been guessed
                if (letter.isupper() and letter.isalpha() and 
                    'A' <= letter <= 'Z' and letter not in guessed_letters):
        ...
        if total > 0:
            for letter, count in letter_counts.items():
                # Double check letter is valid before indexing
                if letter.isupper() and letter.isalpha() and 'A' <= letter <= 'Z':
